...
- **世界自己进化**：不只文笔越写越好（作者层），**世界本身的演化动力**也会自动变得更有戏（模拟层）——资源生态分层、派系叛离自立、剧情张力自调；模拟器甚至能**自创新机制**，经沙箱影子模拟验证后注入活世界。
...
**两层自进化**，都默认开（`NOVEL_EVOLVE`），学的是「经验」不是模型权重，数据落各世界目录、重启不丢。引擎核心 `core/` 不内嵌任何进化逻辑（只读通用 `props.tuning` 数值），守住分离。

### 作者层 · 怎么把模拟「写成章节」

每个世界**每 8 章自动进化一次**（提示上下文 + 采样/引擎参数）：

- **质量-多样性存档（MAP-Elites / QDAIF）**：按「语气 × 节奏」风格网格，每格只留该风格下评分最高的精英基因 → 结构性防「千章一面」的模式坍塌。
- **混合评估**：LLM 评委 rubric（新鲜度/节奏/对白/钩子/连贯/人物）+ 确定性客观指标（重复率/对白占比/词汇多样性/避雷命中）。客观指标只用于打分、**不进生成提示**（防刷分）。
- **进化记忆**：避雷表（套话，带年龄衰减）/ 发扬表（有效写法）/ 重点修正，注入下一卷生成。
- **LLM 提议变异 + novelty 探索**：变异 LLM 据分项反思自由调参；每卷选一个未点亮的风格格作目标、软性 steer 生成去填它。
- **全局传承层**（`global-evolution.json`）：把所有世界进化出的**通用套话避雷 + 最优基因**汇成全局池，**自动播种每个新世界**、并回馈在跑世界——越多世界进化过，新世界起跑线越高。

落盘：`genome.json` / `archive.json` / `evolution.json`。

### 模拟层 · 世界本身「怎么演化」

不止把模拟写得更好看，而是让**世界本身的演化动力**（角色决策 / 派系消长 / 事件触发 / 命运交织）自动更有戏、更涌现：

- **可验证的「有戏」度量（sim-fitness）**：① **story-sifting** 从事件流筛「成链的好故事」（复仇闭环 / 崛起陨落 / 逆袭登顶 / 覆灭复兴 / 巨变连锁）× 惊喜质量（统计罕见度 × 因果链完整度 × 跨度）；② **派系冲突图张力**（势均 / 交锋 / 化解，Ware 冲突四指标）；③ **ASAL 历史新颖度**（抗停滞）。全是可验证符号指标、抗刷分，LLM 仅做校准。
- **可进化的模拟旋钮**：资源稀缺 / 冲突增益 / 大事频率 / 人物代谢 / 生态位分工 / 派系结构生长——MAP-Elites 用 sim-fitness 进化它们；默认值=现状，不破坏在跑世界。
- **涌现底座**（core，genre 中立）：资源**零和竞争**（质量守恒）→ 资源分层 / 寄生 / 生态位分化；社交感知 → 派系**职能分工**；结构生长 → 派系内部离心小团体**叛离自立新派系**（`FactionSplit`）；群像**人口稳态**（防坍塌）。
- **混沌边缘控制器 + 戏剧导演**：监控兴亡密度，太冷加注 / 太热护盘；张力低时「顺水推舟」推进半成形故事链（而非凭空砸大事、读者麻木）。
- **模拟器自创机制**：LLM 提议**全新世界机制** → 静态自洽闸 + 新颖闸 + **沙箱影子模拟闸**（空跑 N tick 不崩 / 人口不坍才准入）→ 注入活世界。模拟器自己长出新玩法。

落盘：`sim-fitness.json` / `sim-fitness-history.json` / `sim-rules.json`。研究综合见 `.audit/20260531-world-simulator-evolution/synthesis.md`。

`GET /api/evolution` 同时返回作者层（存档网格 / 评分曲线 / 全局池）与模拟层（`sim` 段：fitness 曲线 / 筛出的好故事 / 已准入的自创机制）；网页右栏**「世界演化·模拟层」**面板实时展示。

## 🎛️ 世界控制（网页顶栏）

| 按钮 | 作用 | 数据 |
|---|---|---|
| ⏸ 暂停 | 写者在章节边界原地等待（点 ▶ 恢复） | 保留 |
| ⏹ 停止生成 | 结束写者进程（网页仍可读、可重启） | 保留 |
| 🗑 彻底删除 | 结束写者 + 数据归档到 `*-killed` + 移出列表 + 服务自停 | 归档（可恢复） |
...
app/             composition roots：longrun（写者）、server（网页）、gen-world、sandbox
                 自进化：evolve（作者层基因）/ sim-fitness（模拟层度量）/ drama（临界控制器+导演）/ sim-rules（自创机制+影子模拟闸）/ canon / constraints
