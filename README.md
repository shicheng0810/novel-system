...
每个世界**每 8 章自动进化一次**（`NOVEL_EVOLVE` 默认开），学的是「自己的写作经验」（提示上下文 + 采样/引擎参数），不是模型权重：
...
- **全局传承层**（`global-evolution.json`）：把所有世界进化出的**通用套话避雷 + 最优基因**汇成全局池，**自动播种每个新世界**、并回馈在跑世界——越多世界进化过，新世界起跑线越高。已关闭世界的数据仍贡献全局。

进化数据落在各世界目录的 `genome.json` / `archive.json` / `evolution.json`，重启不丢。`GET /api/evolution` 可看存档网格、评分曲线、全局池。引擎核心 `core/` 不内嵌任何进化逻辑（只读通用 `props.tuning` 数值），守住分离。
