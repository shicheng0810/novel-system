...
- 🧠 **越写越好看**——它会自己琢磨怎么把故事讲得更精彩、世界本身怎么演化更有戏。这本事是它**自己练出来的**，你不用管。而且它**跨世界长记性**：一个世界练出的好「打法」（比如怎么让一大群人都活着、戏更密的「群像打法」）会自动**传给下一个同类世界**——你开的世界越多，新世界起跑线越高。
- 🎬 **开篇就在戏里**——新世界起跑前会先**自己悄悄演化一阵**，从里头挑出**最有戏的那条线**，让第一章直接从矛盾最炸的一刻切进去（而不是平淡开头慢慢交代）。
...
```bash
NOVEL_STANDBY=1 NOVEL_SAGA_DIR=mystory PORT=8990 npm run serve:core
```

**③ 打开浏览器 → http://127.0.0.1:8990**

第一次进会有**新手引导**带你填 API Key；填好后，世界**待机中**、会让你**先定义你的世界**：
- 选一个**模板**（宗门争锋 / 重生复仇 / 重生九零 / 赛博东京 / 万族争雄），或
- 自己写**世界描述**，可选填**体系/规则**、**主角**、**成品大纲**，
- 选**用法**：涌现（自由发挥）/ 均衡（推荐）/ 照写（严格跟你的大纲）。

点「开始创造世界」→ 约 1-3 分钟生成并起跑 → 首章落定后页面自动进入，坐等小说一章章长出来。

> 💡 想随时再看引导点右上角 **?**；想再开别的世界点 **✦**（同样一句话或大纲）。各世界独立、互不打扰。
...
| `NOVEL_WARMUP` | 起跑前先静默演化 N tick、再挑最有戏的弧线 in-medias-res 起笔（0=创世即写） | `0` |
| `NOVEL_WORLD_INTENT` | 新世界从全局 QD 存档按引擎策略 niche 取种（别名 `群像`→低代谢×生长，或直接 `低代谢×生长`） | — |
| `NOVEL_BIBLE` | 新世界（无快照）注入自定义 premise（已有世界续用快照、不受影响） | — |
| `NOVEL_EVOLVE` | 自进化开关（`0` 关，作者层+模拟层都默认开） | `1` |
...
- **跨世界 QD 传承层**（`global-evolution.json`）：跨世界共享一个**按「引擎策略」分 niche 的存档**——`turnoverRate × structureGrowth` 分桶（如「低代谢×生长＝群像友好」「高代谢×平＝文笔向」各占一格、每格各留最优引擎），外加通用避雷池。`promoteToGlobal` 逐 niche 单调入库（清掉某世界也不丢它已沉淀的精英），跨世界写入用 `O_EXCL` 锁 + 原子写。新世界可按意图 `NOVEL_WORLD_INTENT=群像` 取**对应 niche 的最优引擎**起步——让一个世界练出的「群像打法」等**局部最优不被单一适应度冠军淹没**、自动传给下一个同类世界（解「搁浅」）。设计与对抗式评审见 `.audit/20260604-global-qd-evolution/`。
...
- **规则层进化（铁律 · 人类策展）**：进化停滞时，LLM 提议**改写一条世界「铁律」**（如「重大转折必有代价」），即改变**叙事可能性空间本身**；这类提案**不走全自动**、必须你在网页议事栏点「**准·改空间 / 驳**」拍板——动剧情可逆、动规则不可逆，故意留人把关（`constraints.json`）。
