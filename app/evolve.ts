...
//   moveBias 自发空间位移偏置(默认0=现状; move 候选无 targetIds 拿不到互动红利 flat+0.3、天生选不中→恒 move=0, 故需评分后偏置破 0; 温情专属经 drama 注入 0.15、爽文默认0 逐字节不变)
export interface EngineGenes { priorWeight: number; scarcity: number; conflictRate: number; eventBias: number; turnoverRate: number; nicheWeight: number; structureGrowth: number; moveBias: number }
...
export const DEFAULT_GENOME: Genome = {
  ...
  engine: { priorWeight: 1.0, scarcity: 0, conflictRate: 1.0, eventBias: 1.0, turnoverRate: 1.0, nicheWeight: 0, structureGrowth: 0, moveBias: 0 }, // 全默认 = 现状行为(不破坏在跑世界)
  generation: 0,
};
