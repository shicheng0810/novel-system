...
export interface Genome {
  ...
  engine: { priorWeight: number }; // 八字/奇门先验对候选动作的引导强度(1.0=现状)
  generation: number;
...
async function mutateGenome(llm: LLMProvider, parent: Genome, reflection: string): Promise<Genome> {
  ...
  try {
    const raw = await llm.complete(
      `你在为小说生成器微调参数。父参数：temperature=${parent.gen.temperature}, frequencyPenalty=${parent.gen.frequencyPenalty}, presencePenalty=${parent.gen.presencePenalty}, priorWeight=${parent.engine.priorWeight}。\n最近评审反馈：${reflection}\n据反馈提议一处小幅调整以提升质量与新鲜度，只动 1-2 个、每个幅度≤0.2。只回 JSON：{"temperature":数,"frequencyPenalty":数,"presencePenalty":数,"priorWeight":数}`,
    ...
    child.engine.priorWeight = clamp(j["priorWeight"], 0.5, 1.6, parent.engine.priorWeight);
  } catch {
...
export async function evolveOnce(llm: LLMProvider, sys: string, dir: string, vol: number, chapters: Array<{ goal: string; text: string }>): Promise<{ genome: Genome; ledger: Ledger; guidance: string; report: string }> {
  ...
  // 选父 + 变异 → 下卷基因
  const reflection = `适应度${fitness}(LLM${llmFit}/客观${objFit})。修正：${c.fixes.join("；") || "无"}。客观：重复率${(m.repetition * 100).toFixed(1)}%、对白${(m.dialogueRatio * 100).toFixed(0)}%、词汇多样${(m.ttr * 100).toFixed(0)}%、命中避雷${m.avoidHits}。${antiProxy ? "⚠长度/重复疑似刷分(已打折)" : ""}`;
