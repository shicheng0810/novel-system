...
export async function generateWorldConfig(prompt: string, llm: LLMProvider, outline?: string): Promise<Record<string, unknown>> {
  const ol = (outline ?? "").trim();
  // 成品大纲: 作为世界「初始设定底座」忠实提取(人物/势力/弧线/大事), 之后引擎仍自行涌现演化(非逐字脚本)。
  const outlineBlock = ol
    ? `\n\n作者另附一份【成品大纲】(可能含人物/势力/情节走向/关键转折)。请把它当作世界的**初始设定底座**：从中忠实提取主角、势力、地点、阶梯与关键大事，并让 arcs 尽量对应大纲的情节脉络；名字与设定要贴合大纲、不要另起炉灶。注意：引擎之后会据此**自行涌现演化**，大纲是起点而非逐字脚本。\n【成品大纲】：\n${ol.slice(0, 4000)}`
    : "";
  const result = await llm.complete(
    `你是"世界设定生成器"。根据用户的世界描述${ol ? "与成品大纲" : ""}，产出一份可直接驱动小说世界引擎的 WorldConfig JSON。\n${SPEC}\n\n用户世界：${prompt}${outlineBlock}\n\n只输出 JSON 本身，不要任何解释、不要 \`\`\` 代码块标记。`,
