...
async function writeChapter(chCtx: ChapterCtx): Promise<{ goal: string; text: string; hook: string }> {
  ...
  const groundLedger = new Map<string, string>(); // [Phase1b·接地] entity→fact 章内 ground-truth 账(first-wins·FACT_GROUND 才填)·喂后续段当既定事实
  ...
  for (let i = 0; i < beats.length; i++) {
    ...
    const groundFacts = FACT_GROUND && groundLedger.size ? `【本章既定事实·下列名/性别已立, 后续段须照此值写, 勿另起新名或改性别(这是已发生的事实, 不是禁令)】${[...groundLedger.values()].join("；")}` : ""; // [Phase1b·接地] 钉前段已立事实当 ground-truth(空则坍缩=golden逐字节同)
    const secPrompt = buildSecPrompt({ sys, n, goal, vol, scene, ambience, crisis, ros, text, prev, beats, covered, metNames, metRoles, seenPlaces, statedLines, factStrict: FACT_V3, weave, sceneAvoid, recentImgs, loreBlock, canonHard, openerFacts: n <= OPENER_CH ? OPENER_FACTS_STR : "", groundFacts, canonInject, conBlock, evoGuidance, voiceCard, secBudget, wrote, perSec, GENTLE, FULLCTX, SECTIONS, MINLEN, PENMANSHIP }, i, last);
    let sec = "";
    ...
    if (GENTLE && (FACT_V2 || FACT_V3)) { // [P1 v2/v3·数据型·喂数据非散文(避 v1 喂散文触发 induction-head 复制)]
      try {
