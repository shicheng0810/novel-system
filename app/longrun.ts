...
async function writeChapter(chCtx: ChapterCtx): Promise<{ goal: string; text: string; hook: string }> {
  ...
  for (let i = 0; i < beats.length; i++) {
    ...
    const secPrompt = buildSecPrompt({ sys, n, goal, vol, scene, ambience, crisis, ros, text, prev, beats, covered, metNames, metRoles, seenPlaces, statedLines, weave, sceneAvoid, recentImgs, loreBlock, canonHard, canonInject, conBlock, evoGuidance, voiceCard, secBudget, wrote, perSec, GENTLE, FULLCTX, SECTIONS, MINLEN, PENMANSHIP }, i, last);
    let sec = "";
