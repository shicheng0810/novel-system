...
export interface SecPromptCtx { sys: string; n: number; goal: string; vol: number; scene: string; ambience: string; crisis: string; ros: string; text: string; prev: string; beats: string[]; covered: string[]; metNames: Set<string>; metRoles: Set<string>; seenPlaces: Set<string>; statedLines?: string[]; weave: string; sceneAvoid: string; recentImgs: string[]; loreBlock: string; canonHard: string; openerFacts?: string; canonInject: string; conBlock: string; evoGuidance: string; voiceCard: string; secBudget: number; wrote: number; perSec: number; GENTLE: boolean; FULLCTX: boolean; SECTIONS: number; MINLEN: number; PENMANSHIP: string }
