...
## 6. Phase C 更狠复测(6臂3×样本)终裁:**不采纳·靶错层**(2026-06-13)
舰队 `verify-opener-v2`(6臂 fresh 冷启·3 A-opener-fix + 3 B-control·各 ch1-6·实际 4-6 章[冷启世界也被环境回收]):

| 轴 | A组(OFS+守卫·13章) | B组(对照·13章) | 判定 |
|---|---|---|---|
| factualContradiction(主) | **2/13(15%)** | 0/13(0%) | A **反多**(OFS 未减·甚至更多) |
| transitionGap | 0/13(0%) | 1/13(8%·B3船未上坐舱) | A 略少(n=1) |
| d1c(副) | 6.0 | 6.8 | A benign |

**A组 factC 命中明细**: A2 ch1(铜钱"磨得发亮"又"新的"·属性矛盾)、A3 ch1(同一次摆渡两版:船钱5↔3文、来历北边投亲↔清波门游学)。
**裁断: 不采纳·OFS 靶错层。** 3×样本逼出了 factC(~15%·证 Phase A 的 17% 真·批1的0是小n运气)——**但真矛盾全是涌现型微事实**(铜钱属性/船钱数值/角色来历·in-medias-res 四段盲写各编一版), **不是 OFS 喂的配置级人名/地点**。OFS 打配置层、病在微事实层=靶错层 → 无效(A 反多=噪声+不对位)。连 FACT_LEDGER=3 数值守卫都没拦住 A3 的5↔3文(段间盲写整场重编·守卫太弱)。
**真问题(本程最大发现)**: 开篇 factC 的根 = **in-medias-res 四段盲写"同一场景各编一版"**(船钱/来历/属性每段自由发挥), 与复述轴的"分段互盲"同源但更难——复述是重复已立事实(FACT_LEDGER v2 治得了), 矛盾是各立冲突事实(需段间共享"已立微事实"且强约束·v3 试了太弱)。
**处置**: OFS/守卫**搁置**(gated 留档·无害但无效·靶错层); 判官扩两类**留 CLI 分诊**(durable 赢面·能抓这些)。真要治需更强的"段间微事实账+矛盾否决"(比 v3 狠: 抽全部数值/属性断言+下段硬约束), 但属下一研究项·非本程能轻量解决。**测量纪律下: 不采纳·诚实记真问题在更深层。**
